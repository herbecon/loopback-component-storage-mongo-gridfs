...
GridFSService.prototype.upload = function (containerName, req, cb) {
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    ...
    if (self.options.watermark_enable && req.headers.watermark === "true") {
      file = file.pipe(sharp().rotate().composite([{ 
        input: self.options.watermark_image, 
        gravity: self.options.watermark_gravity 
      }]));
    }
