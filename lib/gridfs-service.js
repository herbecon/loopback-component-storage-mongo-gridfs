...
GridFSService.prototype.deleteFilesContainerByType = function (container, type, cb) {
  ...
  fs_files.find({ 'metadata.container': container, 'metadata.type': type }, { _id: 1 }).toArray().then(function (containerFiles) {
    var files = [];
...
GridFSService.prototype.uploadWithMetadata = function (containerName, metadata, req, cb) {
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    ...
    stream.on('finish', function(file) {
      // El objeto `file` contiene la información del archivo insertado
      cb(null, this.id);
    });
